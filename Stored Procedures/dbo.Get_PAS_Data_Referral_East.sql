...
CREATE PROCEDURE [dbo].[Get_PAS_Data_Referral_East]
...
SET NOCOUNT ON;
	...
	Declare @LastReferralDateString as date = '1 January 2019'
		...
		--DECLARE @LastReferralDate AS DATE = (SELECT ISNULL(MAX(ReferralDate),'1 April 2017') FROM [Foundation].[dbo].[PAS_Data_Referral] WHERE Area='East')
	...
	EXEC('SELECT DISTINCT
		...
		cast(REFER.TRT_DATE as date) AS DateOfAppointment,
		...
		nullif(rtrim(REFER.ACTNOTEKEY), '''') AS Actnotekey,
		...
		case
			...
			when right(substring(a.text from position (''Spec'' in a.text) for 11),6) in (''101029'', ''101039'') then ''Downgraded''
			when right(refer.spec,3) in (''101029'', ''101039'') then ''Upgraded''
	...
	case 
			...
			when right(substring(a.text from position (''Spec'' in a.text) for 11),6) in (''101029'', ''101039'') then cast(a2.DatDone as date)
			when right(refer.spec,3) in (''101029'', ''101039'') then cast(a2.DatDone as date)
	...
	WHERE 
		...
		and refer.caseno = ''615890''


		--order by refer.datonsys
...
--LEFT JOIN CONSULT CONSULT ON (CONSULT.CONSULTANT_INITIALS = REFER.CONS)
GO
