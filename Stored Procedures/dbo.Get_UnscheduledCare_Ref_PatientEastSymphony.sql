...
CREATE PROCEDURE [dbo].[Get_UnscheduledCare_Ref_PatientEastSymphony]
...
DECLARE @LastAttendanceDate AS DATE = DATEADD(DAY,-7,GETDATE())--(SELECT ISNULL(MAX(ArrivalDate),'31 December 2019') FROM [Foundation].[dbo].[UnscheduledCare_Data_EDAttendance] WHERE Source='WEDS' AND Area='East')-- AND SiteCodeOfTreatment='7A1AU')
...
FROM
	...
	LEFT JOIN dbo.Episodes E ON A.atd_epdid=E.epd_id
WHERE
...
FROM
	...
	LEFT JOIN dbo.Gp GP ON PD.pdt_gpid=GP.gp_id
	LEFT JOIN dbo.Gp_Practise GPPractice ON PD.pdt_practise=GPPractice.Pr_id
...
FROM
	dbo.Aud_Patient_Details APD
	INNER JOIN @Result R ON LocalPatientIdentifier=APD.pdt_pid
	LEFT JOIN dbo.Gp GP ON APD.pdt_gpid=GP.gp_id
	LEFT JOIN dbo.Gp_Practise GPPractice ON APD.pdt_practise=GPPractice.Pr_id
...
SELECT 
	...
	''WEDS'' AS Source,
	''EDA'' AS Dataset,
	CAST(R.Attendanceidentifier AS VARCHAR(20))+''|East|WEDS|EDA'' AS PatientLinkId,
	R.NHSNumberStatus,
	R.DHA,
	R.Ethnicity
FROM 
