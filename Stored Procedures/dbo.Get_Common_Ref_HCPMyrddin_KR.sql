...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[Get_Common_Ref_HCPMyrddin_KR]
AS
BEGIN	
SET NOCOUNT ON;
 
SELECT 
		case when r.LocalNationalCode = n.Maincode then N.MainCode
		...
		p.nadex, 
		p.PractitionerType, 
		(p.MainCode +' - '+ p.Name) as CodeName, 
		(p.Name  +' - '+ p.MainCode) as NameCode, 
		r.Active, 
		p.CHKS, 
		P.CapacityPlanning, 
		r.LocalNationalCode, 
...
Left Join [7A1A1SRVINFODW3].[dbo].[Reference_NWIS_HCP] N on N.MainCode = r.LocalNationalCode
