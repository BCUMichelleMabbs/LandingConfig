...
CREATE PROCEDURE [dbo].[Get_PAS_Data_Inpatient_East]
...
DECLARE @DateEpisodeEnded AS DATE = (SELECT ISNULL(MAX(DateEpisodeEnded),'1 January 2017') FROM [Foundation].[dbo].[PAS_Data_Inpatient] where Area = 'East')
DECLARE @DateEpisodeEndedString AS VARCHAR(30) = DATENAME(DAY,@DateEpisodeEnded) + ' ' + DATENAME(MONTH,@DateEpisodeEnded) + ' ' + DATENAME(YEAR,@DateEpisodeEnded)

--DECLARE @DateAdmitted AS DATE = (SELECT ISNULL(MAX(DateAdmitted),'1 January 2019') FROM [Foundation].[dbo].[PAS_Data_Inpatient] where Area = 'East')
...
EXEC( 'SELECT DISTINCT
		...
		--nullif(rtrim(t.postcode), '''') as PostcodeAtTimeOfActivity,
		...
		extract(YEAR from t.trt_date)||''-''||extract(month from t.trt_date)||''-''||extract(DAY from t.trt_date) as DateAdmitted,
		...
		extract(YEAR from t.disdate)||''-''||extract(month from t.disdate)||''-''||extract(DAY from t.disdate) as DateDischarged,
		...
		extract(YEAR from e.start_date)||''-''||extract(month from e.start_date)||''-''||extract(DAY from e.start_date) as DateEpisodeStarted,
		
		case 
				when e.end_date = ''31 December 2999'' then null
				else extract(YEAR from e.end_date)||''-''||extract(month from e.end_date)||''-''||extract(DAY from e.end_date) 
		...
		extract(YEAR from c1.thedate)||''-''||extract(month from c1.thedate)||''-''||extract(DAY from c1.thedate) as DateOfProcedure1,
		nullif(rtrim(c2.thecode), '''') as Procedure2,
		extract(YEAR from c2.thedate)||''-''||extract(month from c2.thedate)||''-''||extract(DAY from c2.thedate) as DateOfProcedure2,
		nullif(rtrim(c3.thecode), '''') as Procedure3,
		extract(YEAR from c3.thedate)||''-''||extract(month from c3.thedate)||''-''||extract(DAY from c3.thedate) as DateOfProcedure3,
		nullif(rtrim(c4.thecode), '''') as Procedure4,
		extract(YEAR from c4.thedate)||''-''||extract(month from c4.thedate)||''-''||extract(DAY from c4.thedate) as DateOfProcedure4,
		nullif(rtrim(c5.thecode), '''') as Procedure5,
		extract(YEAR from c5.thedate)||''-''||extract(month from c5.thedate)||''-''||extract(DAY from c5.thedate) as DateOfProcedure5,
		nullif(rtrim(c6.thecode), '''') as Procedure6,
		extract(YEAR from c6.thedate)||''-''||extract(month from c6.thedate)||''-''||extract(DAY from c6.thedate) as DateOfProcedure6,
		nullif(rtrim(c7.thecode), '''') as Procedure7,
		extract(YEAR from c7.thedate)||''-''||extract(month from c7.thedate)||''-''||extract(DAY from c7.thedate) as DateOfProcedure7,
		nullif(rtrim(c8.thecode), '''') as Procedure8,
		extract(YEAR from c8.thedate)||''-''||extract(month from c8.thedate)||''-''||extract(DAY from c8.thedate) as DateOfProcedure8,
		nullif(rtrim(c9.thecode), '''') as Procedure9,
		extract(YEAR from c9.thedate)||''-''||extract(month from c9.thedate)||''-''||extract(DAY from c9.thedate) as DateOfProcedure9,
		nullif(rtrim(c10.thecode), '''') as Procedure10,
		extract(YEAR from c10.thedate)||''-''||extract(month from c10.thedate)||''-''||extract(DAY from c10.thedate) as DateOfProcedure10,
		nullif(rtrim(c11.thecode), '''') as Procedure11,
		extract(YEAR from c11.thedate)||''-''||extract(month from c11.thedate)||''-''||extract(DAY from c11.thedate) as DateOfProcedure11,
		nullif(rtrim(c12.thecode), '''') as Procedure12,
		extract(YEAR from c12.thedate)||''-''||extract(month from c12.thedate)||''-''||extract(DAY from c12.thedate) as DateOfProcedure12,
		...
		extract(YEAR from t.next_approx_appt)||''-''||extract(month from t.next_approx_appt)||''-''||extract(DAY from t.next_approx_appt) as DateOfNextApproximateAppointment,
		extract(YEAR from t.est_disdate)||''-''||extract(month from t.est_disdate)||''-''||extract(DAY from t.est_disdate) as DateEstimatedOfDischarge,
		...
		--cant use below as its used for both current and discharging codes
...
(
	(
		...
		(
			...
			E.END_DATE > '''+@DateEpisodeEndedString+'''
		)
	)
...
-- (t.trt_type in (''AN'', ''AT'') and t.trt_date >= '''+ @Date + ''')
