...
CREATE PROCEDURE [dbo].[PCNSX_MinorsRepeatEDAttendancesYG]
AS
BEGIN
	SET NOCOUNT ON;
...
SELECT Id, Value, RecipientTypeId FROM [7A1A1SRVINFODW1].PCNS.dbo.Recipient R WHERE R.GroupId = 63 AND Active='Y' AND RecipientTypeId=1  --63 is the group id for the temporary ed group
					...
					WHILE @ThisCurrentRecipient <= @ThisMaxRecipient
						BEGIN
							...
							INSERT INTO [7A1A1SRVINFODW1].PCNS.dbo.[Log] (
								...
								VALUES(0,@RecipientId,@RecipientTypeId,13,1,12,@ThisLPI,'West','Symphony','Emergency Department attendance','Emergency Department','Bangor Hospital',@ThisArrDate,GetDate(),'Sent',null)
								SET @ThisCurrentRecipient+=1
						END
