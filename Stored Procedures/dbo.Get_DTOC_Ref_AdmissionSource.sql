...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[Get_DTOC_Ref_AdmissionSource]
AS
BEGIN
SET NOCOUNT ON;
...
Set @Server += '.[DTOCS].[dbo].[VW_HealthBoard_AdmissionSources] '   -- Concatinate server name with table into one parameter
Set @SQL2 = 'Where SOA_CODE not in (''1'',''0'') ) SELECT * FROM @Results order by Source,Name'
