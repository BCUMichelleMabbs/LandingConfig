...
SET ANSI_NULLS ON
GO
CREATE Procedure [dbo].[Get_DTOC_Data_Historic] 
...
Set @Server += '.[DTOCS].[dbo].[VW_Healthboard_PreviousDelays] d'   -- Concatinate server name with table into one parameter
Set @Join = ' LEFT JOIN ' + @Server1 + '.[DTOCS].[dbo].[VW_Healthboard_PreviousDelayReasons] r on r.DelayID = d.DelayID ' -- Concatinate server name with joining table
