...
-- =============================================
CREATE PROCEDURE [dbo].[Get_PAS_Data_Outpatient_East]
...
BEGIN
	...
	DECLARE @LastTreatmentDate AS DATE = (SELECT ISNULL(MAX(AppointmentDate),'01 January 2010') FROM [Foundation].[dbo].[PAS_Data_Outpatient] WHERE Source ='Myrddin' and AppointmentDate <> '2999-12-31' and AppointmentDate < CONVERT(date,getdate()))
	...
	ELSE ''1'' END as AttendanceCategory,
     ...
     (P.ACTNOTEKEY = T.ACTNOTEKEY)
		...
		WHERE
			...
			and T.caseno not in (''123456'',''TestWCP4'',''WPASEast2'')
