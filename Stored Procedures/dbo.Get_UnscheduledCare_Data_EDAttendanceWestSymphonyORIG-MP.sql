...
CREATE PROCEDURE [dbo].[Get_UnscheduledCare_Data_EDAttendanceWestSymphonyORIG-MP]
...
DECLARE @LastAttendanceDate AS DATE = (SELECT ISNULL(MAX(ArrivalDate),'31 December 2009') FROM [Foundation].[dbo].[UnscheduledCare_Data_EDAttendance] WHERE Area='West' AND Source='WEDS')
...
declare @Diagn int
set @flm_mtid = (select top 1 mt_id from [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.mappingtypes where mt_name = 'EDDS')
set @NHSid =(select top 1 lkp_id from [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.Lookups where lkp_name = 'NHSNumber')



set @Diagn = (select top 1 rps_value from [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.cfg_reportsettings where rps_name = 'Diagnosis')
...
FROM
	[BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.Request_Details R
	INNER JOIN [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.Attendance_Details A ON R.req_atdid=A.atd_id
WHERE
	R.req_depid IN (Select rps_value from [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.cfg_reportsettings where rps_name = 'Radiology') AND 
...
select res_atdid, res_date, res_field6 from [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.result_details
inner join [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.attendance_details on atd_id = res_atdid
...
select res_atdid, res_date, res_field6 from [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.result_details
inner join [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.attendance_details on res_atdid = atd_id 
where res_depid in (Select rps_value from [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.cfg_reportsettings where rps_name = 'Treatment')
and res_inactive = 0 
and CAST(atd_arrivaldate AS DATE)> ''+@LastAttendanceDateString+'' AND CAST(atd_arrivaldate AS DATE) < ''+@DateToString+''
	
insert  @TemporaryTreat_Disch (res_atdid,res_date,res_result)				
select res_atdid, res_date, res_result from [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.result_details
inner join [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.attendance_details on res_atdid = atd_id 
where res_depid in (Select rps_value from [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.cfg_reportsettings where rps_name = 'Treatment')
and res_inactive = 0 
and CAST(atd_arrivaldate AS DATE)> ''+@LastAttendanceDateString+'' AND CAST(atd_arrivaldate AS DATE) < ''+@DateToString+''
...
FROM
	[BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.Result_details R
	INNER JOIN [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.Attendance_Details A ON R.res_atdid=A.atd_id
WHERE
	R.res_depid IN (Select rps_value from [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.cfg_reportsettings where rps_name = 'DIAGNOSIS') AND 
...
SELECT
	...
	Value AS Treatment
FROM
	[BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.Triage T
	INNER JOIN [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.Attendance_Details A ON T.tri_atdid=A.atd_id
...
SELECT DISTINCT
	...
	CASE 
		WHEN PermAdd.add_postcode IS NULL THEN (SELECT TOP 1 left(ha_pcg,3) FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.ha_pcg WHERE TempAdd.add_postcode=ha_pcg.ha_postcode )
		ELSE(SELECT TOP 1 LEFT(ha_pcg,3) FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.ha_pcg WHERE PermAdd.add_postcode=ha_pcg.ha_postcode )
	END AS DHA,
	...
	CAST((SELECT TOP 1 tri_date FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.triage WHERE tri_atdid = a.atd_id AND tri_inactive <> 1 ORDER BY tri_date ASC) AS DATE) AS TriageStartDate, 
	CAST((SELECT TOP 1 tri_date FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.triage WHERE tri_atdid = a.atd_id AND tri_inactive <> 1 ORDER BY tri_date ASC) AS TIME) AS TriageStartTime,
	NULL AS TriageEndDate,
	NULL AS TriageEndTime, 
	CAST((SELECT TOP 1 res_date FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.result_details WHERE res_depid IN (SELECT rps_value FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.cfg_reportsettings WHERE rps_name = 'SeeA+EClinicianOutcome') AND res_atdid = a.atd_id AND res_inactive<>1) AS DATE) AS EDClinicianSeenDate,
	CAST((SELECT TOP 1 res_date FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.result_details WHERE res_depid IN (SELECT rps_value FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.cfg_reportsettings WHERE rps_name = 'SeeA+EClinicianOutcome') AND res_atdid = a.atd_id AND res_inactive<>1) AS TIME) AS EDClinicianSeenTime,
	...
	CAST((SELECT TOP (1) res_date FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.Result_details WHERE (res_depid = (SELECT dep_id FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.CFG_DEProcedures WHERE (dep_Caption = 'Breach Exception'))) AND res_atdid = a.atd_id AND res_inactive <> 1) AS DATE) AS TreatmentCompleteDate,
	CAST((SELECT TOP (1) res_date FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.Result_details WHERE (res_depid = (SELECT dep_id FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.CFG_DEProcedures WHERE (dep_Caption = 'Breach Exception'))) AND res_atdid = a.atd_id AND res_inactive <> 1) AS TIME) AS TreatmentCompleteTime,
	...
	CAST((SELECT TOP (1) req_date FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.Request_Details WHERE (req_depid IN (SELECT dep_id FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.CFG_DEProcedures WHERE (dep_Caption = 'DTA'))) AND req_atdid = a.atd_id AND req_inactive <> 1) AS DATE) AS DecisionToAdmitDate,
	CAST((SELECT TOP (1) req_date FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.Request_Details WHERE (req_depid IN (SELECT dep_id FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.CFG_DEProcedures WHERE (dep_Caption = 'DTA'))) AND req_atdid = a.atd_id AND req_inactive <> 1) AS TIME) AS DecisionToAdmitTime,
	...
	NULLIF(CAST((
		CASE a.atd_attendancetype 
			...
			ELSE (SELECT TOP 1 cul_locationdate FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.current_locations WHERE cul_locationid IN (SELECT TOP 1 loc_id FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.cfg_locations WHERE loc_name = 'Left Department') AND cul_atdid = a.atd_id ORDER BY cul_locationdate DESC)
		END
	...
	CAST((SELECT TOP (1) res_date FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.Result_details AS Trans WHERE (res_depid IN (SELECT rps_value FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.CFG_ReportSettings AS CFG_ReportSettings_5 WHERE (rps_name = 'Transport'))) AND res_atdid = A.atd_id AND res_inactive <> 1) AS DATE) AS TransportRequestedDate,
	CAST((SELECT TOP (1) res_date FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.Result_details AS Trans WHERE (res_depid IN (SELECT rps_value FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.CFG_ReportSettings AS CFG_ReportSettings_5 WHERE (rps_name = 'Transport'))) AND res_atdid = A.atd_id AND res_inactive <> 1) AS TIME) AS TransportRequestedTime,
	NULL AS TransportArrivedDate,  
	NULL AS TransportArrivedTime,
	CAST((SELECT TOP (1) req_date FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.Request_Details WHERE (req_depid IN (SELECT rps_value FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.CFG_ReportSettings WHERE (rps_name = 'A+E BedRequest'))) AND req_atdid = a.atd_id AND req_inactive <> 1) AS DATE) AS BedRequestedDate,
	CAST((SELECT TOP (1) req_date FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.Request_Details WHERE (req_depid IN (SELECT rps_value FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.CFG_ReportSettings WHERE (rps_name = 'A+E BedRequest'))) AND req_atdid = a.atd_id AND req_inactive <> 1) AS TIME) AS BedRequestedTime,
	CAST((SELECT TOP (1) res_date FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.Result_details WHERE (res_depid IN (SELECT rps_value FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.CFG_ReportSettings WHERE (rps_name = 'A+E BedRequestOutcome'))) AND res_atdid = a.atd_id AND res_inactive <> 1) AS DATE) AS BedRequestedOutcomeDate,
	CAST((SELECT TOP (1) res_date FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.Result_details WHERE (res_depid IN (SELECT rps_value FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.CFG_ReportSettings WHERE (rps_name = 'A+E BedRequestOutcome'))) AND res_atdid = a.atd_id AND res_inactive <> 1) AS TIME) AS BedRequestedOutcomeTime,
	...
	NULLIF(CASE a.atd_attendancetype 
		WHEN 1 THEN ISNULL(NULLIF(
			(select distinct not_text
				from 
				[BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.attendance_details innerA2
				inner join [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.episodes on atd_epdid = epd_id 
				inner join [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.lookupmappings on flm_lkpid = atd_reason
				inner join [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.notes on not_noteid = flm_value
			where 
			...
			(select distinct not_text
				from 
				[BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.attendance_details innerA1
				inner join [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.episodes on atd_epdid = epd_id 
				inner join [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.lookupmappings on flm_lkpid = atd_reason
				inner join [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.notes on not_noteid = flm_value
			where 
		...
		ELSE LEFT((select top 1 not_text from [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.notes where not_noteid = (select top 1 flm_value from [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.lookupmappings where flm_lkpid = a.atd_reason and flm_mtid = 6)), 2) 
	end,'') as PatientGroup,
	...
	CASE
		WHEN (select not_text from [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.notes where not_noteid = (select top 1 flm_value from [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.lookupmappings where flm_lkpid = a.atd_reason and flm_mtid = @flm_mtid)) not in (11,12,13,14,15) THEN '98'
		ELSE
		ISNULL(
			CASE
				WHEN (select top 1 lkp_name from [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.lookups where lkp_id = a.atd_activity) in 
				...
				WHEN (select top 1 lkp_name from [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.lookups where lkp_id = a.atd_activity) = 'Miscellaneous' THEN '08'
				WHEN (select top 1 lkp_name from [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.lookups where lkp_id = (select top 1 rta_patientype from [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.RTA_Details where rta_atdid = a.atd_id)) in
				...
				WHEN (select top 1 lkp_name from [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.lookups where lkp_id = a.atd_location) = 'Work Place' THEN '01'
				WHEN (select top 1 lkp_name from [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.lookups where lkp_id = a.atd_location) = 'Educational Establishment' THEN '02'
				WHEN (select top 1 lkp_name from [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.lookups where lkp_id = a.atd_location) in
				...
				WHEN (select top 1 lkp_name from [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.lookups where lkp_id = a.atd_location) = 'UNKNOWN' THEN '98'
				WHEN (select top 1 lkp_name from [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.lookups where lkp_id = a.atd_location) in
	...
	(SELECT TOP 1 tri_category FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.triage WHERE tri_atdid = a.atd_id AND tri_inactive <> 1 ORDER BY tri_date ASC) AS TriageCategory, 

	NULLIF((SELECT TOP 1 tri_complaint FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.triage WHERE tri_atdid = a.atd_id AND tri_inactive <> 1 ORDER BY tri_date ASC),'') AS TriageComplaint, 
	NULLIF(RTRIM((SELECT TOP 1 tri_treatment FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.triage WHERE tri_atdid = a.atd_id AND tri_inactive <> 1 ORDER BY tri_date ASC)),'') AS TriageTreatment,
	...
	NULLIF(RTRIM((SELECT TOP 1 tri_discriminator FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.triage WHERE tri_atdid = a.atd_id AND tri_inactive <> 1 ORDER BY tri_date ASC)),'') AS TriageDiscriminator, 
	...
	CASE
		...
		WHEN (
			SELECT 
				TOP 1 res_result 
			FROM  [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.Result_details 
				INNER JOIN [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.lookups ON lkp_id = res_result 
			WHERE 
					res_inactive<>1 AND 
	...
	(SELECT TOP (1) req_field1 FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.Request_Details WHERE (req_depid IN (SELECT rps_value FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.CFG_ReportSettings WHERE (rps_name = 'A+E BedRequest'))) AND req_atdid = a.atd_id AND req_inactive <> 1) AS BedRequestedWard,
	NULL AS BedRequestedConsultant,
	(SELECT TOP (1) req_request FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.Request_Details WHERE (req_depid IN (SELECT rps_value FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.CFG_ReportSettings WHERE (rps_name = 'A+E BedRequest'))) AND req_atdid = a.atd_id AND req_inactive <> 1) AS BedRequestedSpecialty,
	(SELECT TOP (1) res_result FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.Result_details WHERE (res_depid IN (SELECT rps_value FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.CFG_ReportSettings WHERE (rps_name = 'A+E BedRequestOutcome'))) AND res_atdid = a.atd_id AND res_inactive <> 1) AS BedRequestedOutcome,
	(SELECT TOP 1 res_staff1 FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.result_details WHERE res_depid IN (SELECT rps_value FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.cfg_reportsettings WHERE rps_name = 'SeeA+EClinicianOutcome') AND res_atdid = a.atd_id AND res_inactive<>1) AS EDClinicianSeen,
	CASE a.atd_attendancetype
		WHEN 1 THEN 'Y'
		ELSE 'N'
	END AS SeeAndTreat,
...
FROM 
	...
	INNER JOIN [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.Patient_details on pdt_pid=pat_pid 
	--LEFT JOIN Patient_Details_ExtraFields PDEF on patient.pat_pid=PDEF.pdt_EF_pdtpid and PDEF.pdt_EF_FieldID = 1050 --School
	LEFT OUTER JOIN [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.GPPracticeAddressLink GPLINK on pdt_gpid = GPLINK.gpa_gpid 
	   AND GPLINK.gpa_status = 2661   -- Status = Active
	   AND gpa_prid = pdt_practise   -- link on GP Practice as well
	LEFT OUTER JOIN [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.GP_practise GPPRAC on GPPRAC.pr_id = GPLINK.gpa_prid
	LEFT OUTER JOIN [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.GP PATGP on PATGP.gp_id = GPLINK.gpa_gpid
	LEFT OUTER JOIN [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.Address GPAdd on GPLINK.gpa_praddid = GPAdd.add_addid 
		AND GPAdd.add_linktype = 2 
		AND GPAdd.add_type IN 
			(
				(SELECT TOP 1 Lkp_ID FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.lookups WHERE Lookups.Lkp_Name = 'Principle Practice Address' AND lkp_parentid = (SELECT TOP 1 lkp_id FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.lookups WHERE lkp_name = 'Practice Address Type' AND lkp_parentid = 0)),
				(SELECT TOP 1 Lkp_ID FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.lookups WHERE Lookups.Lkp_Name = 'Secondary Practice Address' AND lkp_parentid = (SELECT TOP 1 lkp_id FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.lookups WHERE lkp_name = 'Practice Address Type' AND lkp_parentid = 0)),
				(SELECT TOP 1 Lkp_ID FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.lookups WHERE Lookups.Lkp_Name = 'GP Address'))
	LEFT OUTER JOIN [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.GP REFGP ON REFGP.gp_id = a.atd_gpid
	LEFT OUTER JOIN [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.GP_Practise REFGPPRAC ON REFGPPRAC.Pr_id = a.atd_prcode
	LEFT OUTER JOIN [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.Address ContAdd ON ContAdd.add_linkid = pat_pid AND ContAdd.add_linktype = 1 AND ContAdd.add_type = (SELECT TOP 1 Lkp_ID FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.lookups WHERE Lookups.Lkp_Name = 'Contact Address')
	LEFT OUTER JOIN [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.Address TempAdd ON TempAdd.add_linkid = pat_pid AND TempAdd.add_linktype = 1 AND TempAdd.add_type = (SELECT TOP 1 Lkp_ID FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.lookups WHERE Lookups.Lkp_Name = 'Temporary address')
	LEFT OUTER JOIN [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.Address PermAdd ON PermAdd.add_linkid = pat_pid AND PermAdd.add_linktype = 1 AND PermAdd.add_type = (SELECT TOP 1 Lkp_ID FROM [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.lookups WHERE Lookups.Lkp_Name = 'Permanent address')
	LEFT JOIN [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.RTA_Details RTA ON a.atd_id=RTA.rta_atdid
	LEFT JOIN [BCUED\BCUED_DB].EMIS_SYM_BCU_Live.dbo.Attendance_Details_ExtraFields MECH ON a.atd_id=MECH.atd_EF_atdid AND MECH.atd_EF_FieldID=1070
