...
CREATE PROCEDURE [dbo].[Get_PAS_Ref_Patient_OutpatientWest]
...
DECLARE @LastAttendanceDate AS DATE = (SELECT 
													...
													FROM [Foundation].[dbo].[PAS_Data_Outpatient] WHERE source='pims' and AppointmentDate <> '2999-12-31' and AppointmentDate < CONVERT(date,getdate()))
...
where
(
-- to match outpatient extract
sch.SCTYP_REFNO = 12186
and ISNULL(sch.archv_flag,''N'') = ''N''
and CONVERT(Date,sch.start_Dttm)> '''+ @LastAttendanceDate +'''
--and CONVERT(Date,sch.start_Dttm)>= ''+ 01 january 2010''
...
FROM
	...
	--where localpatientidentifier = ''D284127''
	--where attendanceidentifier = ''1010772''
