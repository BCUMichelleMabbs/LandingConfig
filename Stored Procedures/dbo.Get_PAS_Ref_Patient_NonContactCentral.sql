...
CREATE PROCEDURE [dbo].[Get_PAS_Ref_Patient_NonContactCentral]
...
DECLARE @LastAttendanceDate AS DATE = (SELECT 
													...
													FROM [Foundation].[dbo].[PAS_Data_NonContact] WHERE source='WPAS' and AppointmentDate <> '2999-12-31' and AppointmentDate < CONVERT(date,getdate()))
...
''
	SELECT DISTINCT
		...
		''''WPAS'''' AS Source,
		''''NC'''' AS Dataset,
		T.actnotekey||''''|WPAS|NC'''' AS PatientLinkId,
