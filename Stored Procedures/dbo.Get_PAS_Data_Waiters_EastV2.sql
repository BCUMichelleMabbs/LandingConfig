...
-- Running SP for Parameters:
--            : 21/31/41       - Executes and data displayed within few minutes
--            : 21/31/41/FU/PP - Executes and data displayed within 43 minutes
--
-- Amend Date:  03/06/20   - HWL :This script compiles and executes data but doesn't load direct into foundation
--              Changing this to a select query alone will not work as data extracted from view REF_WAIT_LEN_VIEW_ENH
-- =============================================
CREATE PROCEDURE [dbo].[Get_PAS_Data_Waiters_EastV2]
...
'' 
		...
		r.dat_ref as ClinicalReferralDate,
		c.THECODE as Coding,
		''''OP'''' as WLTYPE,
		cast(''''Now'''' as date),
		...
		cast(pm.pathway_notes as varchar (3000)) as Notes
...
/* -- this is the original version - not loading
...
(
  ...
  FAXNO							VARCHAR(20),
  DOC_REFERENCE_NO				VARCHAR(20),		-- check kr want?
...
(
  ...
  LAST_EVENT_LOCALITY			varchar(100),  				--not on the myrddin version of the SP
  ...
  ANAESTHETIC_TYPE				varchar(50),		--3647 --not on the myrddin version of the SP
  PLANNED_ASA_GRADE				varchar(50),		--6141 --not on the myrddin version of the SP
  ...
  ARMED_SERVICES_KEYNOTE        varchar(200),       --6121
...
--
--EXEC(' Select * from REF_WAIT_LEN_VIEW_ENH (''21'', ''21 april 2020'' ,'''','''','''','''') ') AT [WPAS_East] 
--


--SELECT top 5000 * from @results


SELECT  * from @results

--create table hwl_Eastv2 as select * from @results
