...
--              17/08/2021 TF date parameter  '+ @TodayDateText +'
-- =============================================


CREATE PROCEDURE [dbo].[Get_PAS_Data_Waiters_East_z1_FU1]
...
DECLARE @LastDateWaitingListCensus AS DATE = (SELECT ISNULL(MAX(DateWaitingListCensus),'01 January 2018') FROM [Foundation].[dbo].[PAS_Data_WaitingList] where Area='East')
...
orig.source,
orig.next_appt_freq,
pl.provider_code as next_appt_loc,
orig.outcome,
orig.create_user,
orig.create_date,
orig.last_modify_user,
orig.last_modify_date,
psch.school_code
	...
	from 
		...
		left join treatmnt orig on re.actnotekey = orig.actnotekey
        left join padloc pl on orig.next_appt_loc = pl.loccode
        left join patient_school psch on pat.caseno = psch.caseno and psch.end_date is null
