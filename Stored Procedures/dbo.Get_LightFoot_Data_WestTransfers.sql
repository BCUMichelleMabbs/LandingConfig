...
CREATE PROCEDURE [dbo].[Get_LightFoot_Data_WestTransfers]
AS
BEGIN

--Written by JH NDR
--Amended dates from 01 June 21 to yesterday as per Centrals sp
...
where 
pv.admit_Dttm between '01 June 2021' AND DATEADD(DAY,-1,GETDATE()) --added in mm 01 sep 2021
...
then ISNULL(LAG(Clinician,1) over (Partition by SpellNo order by TransferDate asc),LAG(Clinician,2) over (Partition by SpellNo order by TransferDate asc)) 
else Clinician 
END As Consultant
,ISNULL(ISNULL(ISNULL(LAG(Ward,1) Over (Partition by SpellNo order by TransferDate asc),LAG(Ward,2) Over (Partition by SpellNo order by TransferDate asc)),LAG(Ward,3) Over (Partition by SpellNo order by TransferDate asc)),LAG(Ward,4) Over (Partition by SpellNo order by TransferDate asc)) as PreviousWard
,Ward 
,ISNULL(ISNULL(ISNULL(LEAD(Ward,1) Over (Partition by SpellNo order by TransferDate asc),LEAD(Ward,2) Over (Partition by SpellNo order by TransferDate asc)),LEAD(Ward,3) Over (Partition by SpellNo order by TransferDate asc)),LEAD(Ward,4) Over (Partition by SpellNo order by TransferDate asc)) as NextWard
,CASE WHEN Specialty is null 
then ISNULL(LAG(Specialty,1) over (Partition by SpellNo order by TransferDate asc),LAG(Specialty,2) over (Partition by SpellNo order by TransferDate asc)) 
else Specialty
END As Specialty
...
,CONVERT(varchar,SpellNumber)+'|'+'West'+'|'+CONVERT(varchar,ISNULL(ISNULL(ISNULL(EpisodeNumber,LAG(EpisodeNumber,1) Over (Partition by SpellNumber,Consultant Order by CAST(EventStartDate as DATETIME) + CAST(EventStartTime as DATETIME) Asc)),LAG(EpisodeNumber,2) Over (Partition by SpellNumber,Consultant Order by CAST(EventStartDate as DATETIME) + CAST(EventStartTime as DATETIME) Asc)),LAG(EpisodeNumber,3) Over (Partition by SpellNumber,Consultant Order by CAST(EventStartDate as DATETIME) + CAST(EventStartTime as DATETIME) Asc)))+'|'+'PIMS'+'|'+'IPE' as PatientLinkIDEpisode 
,AdmissionMethod 
,DischargeMethod 
,EventStartDate 
,EventStartTime 
