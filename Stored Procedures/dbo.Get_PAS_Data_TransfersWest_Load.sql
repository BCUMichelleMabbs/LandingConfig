...
-- ================================================================
-- Author:		Ola Oladunjoye
-- Create date: 17th August 2021
-- Description:	Extract of all episode and ward transfers PIMs West
-- ================================================================
CREATE PROCEDURE [dbo].[Get_PAS_Data_TransfersWest_Load]
...
--Written by JH NDR
--Amended dates from 01 June 17 to yesterday as per Centrals sp

DECLARE @DateOfTransfer AS DATE = (SELECT ISNULL(MAX(EventEndDate),'1 January 2017') FROM [Foundation].[dbo].[PAS_Data_Transfers] where Area = 'West')
...
where 
CAST(pv.admit_Dttm AS DATE) > @DateOfTransferString   --added in mm 01 sep 2021
...
,SpellNumber
,ISNULL(ISNULL(ISNULL(ISNULL(ISNULL(ISNULL(ISNULL(EpisodeNumber,LAG(EpisodeNumber,1) Over (Partition by SpellNumber,Consultant Order by CAST(EventStartDate as DATETIME) + CAST(EventStartTime as DATETIME) Asc)),LAG(EpisodeNumber,2) Over (Partition by SpellNumber,Consultant Order by CAST(EventStartDate as DATETIME) + CAST(EventStartTime as DATETIME) Asc)),LAG(EpisodeNumber,3) Over (Partition by SpellNumber,Consultant Order by CAST(EventStartDate as DATETIME) + CAST(EventStartTime as DATETIME) Asc)),LAG(EpisodeNumber,4) Over (Partition by SpellNumber,Consultant Order by CAST(EventStartDate as DATETIME) + CAST(EventStartTime as DATETIME) Asc)),LAG(EpisodeNumber,5) Over (Partition by SpellNumber,Consultant Order by CAST(EventStartDate as DATETIME) + CAST(EventStartTime as DATETIME) Asc)),LAG(EpisodeNumber,6) Over (Partition by SpellNumber,Consultant Order by CAST(EventStartDate as DATETIME) + CAST(EventStartTime as DATETIME) Asc)),LAG(EpisodeNumber,7) Over (Partition by SpellNumber,Consultant Order by CAST(EventStartDate as DATETIME) + CAST(EventStartTime as DATETIME) Asc)) As EpisodeNo
...
,Source
,LocalPatientIdentifier
,SpellNumber as ProviderSpellNumber
,'0'+CONVERT(varchar,DENSE_RANK() Over (Partition by SpellNumber order by SpellNumber,EpisodeNo)) as EpisodeNumber
--,'0' + CONVERT(varchar,ROW_NUMBER() over (Partition by SpellNumber,EpisodeNo order by CAST(EventStartDate as Datetime) + CAST(EventStartTime as Datetime))) as EpisodeNo1
