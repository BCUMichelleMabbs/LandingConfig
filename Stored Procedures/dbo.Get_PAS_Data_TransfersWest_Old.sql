...
-- ================================================================
-- Author:		Jake Hammer
-- Create date:     07th September 2021
-- Description:	Extract of all episode and ward transfers PIMs West
-- ================================================================
CREATE PROCEDURE [dbo].[Get_PAS_Data_TransfersWest_Old]
...
,LocalPatientIdentifier
,SpellNumber
,CONVERT(varchar,ROW_NUMBER() over (Partition by LocalPatientIdentifier,SpellNumber,EpisodeNo order by CAST(EventStartDate as Datetime) + CAST(EventStartTime as Datetime))) as EpisodeNo
-- ,CONVERT(varchar,SpellNumber)+'|'+'West'+'|'+'|'+CONVERT(varchar,ROW_NUMBER() over (Partition by LocalPatientIdentifier,SpellNumber,EpisodeNo order by CAST(EventStartDate as Datetime) + CAST(EventStartTime as Datetime)))+'|'+'PIMS'+'|'+'IPE' as PatientLinkIDEpisode 
,AdmissionMethod 
,DischargeMethod 
,EventStartDate 
,EventStartTime 
