...
CREATE PROCEDURE [dbo].[Get_PAS_Ref_Patient_NonContactEast]
...
/*
NOTE: this data is loaded using a merge process, this means if the patients changes from being an None Face to Face Contact then any records picked up in the process will remain in the patients table, resulting in extra patient in the patients table.
...
DECLARE @LastAttendanceDate AS DATE = (SELECT 
													case	--when max(appointmentdate) = '31 december 2999' then is null
															when max(LoadDate) = convert(date, getdate()) then (select min(AppointmentDate) from [Foundation].[dbo].[PAS_Data_NonContact] group by LoadDate having max(LoadDate) = convert(date, getdate()) )	
															else  ISNULL(max(AppointmentDate),'01 January 2010') 
															end
													FROM [Foundation].[dbo].[PAS_Data_NonContact] WHERE source='Myrddin'and AppointmentDate <> '2999-12-31' and AppointmentDate < CONVERT(date,getdate()))
...
''
	SELECT DISTINCT
		...
		''''Myrddin'''' AS Source,
		''''NC'''' AS Dataset,
		T.actnotekey||''''|Myrddin|NC'''' AS PatientLinkId,
	...
	WHERE
			T.TRT_DATE >= ''''' +  @LastAttendanceDateString + ''''' AND 
			--T.TRT_DATE >= '''' 01 January 2010 '''' and
