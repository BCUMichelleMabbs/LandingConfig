...
CREATE PROCEDURE [dbo].[Get_Radiology_Data_ActivityWest]
...
;
/*
	exec [dbo].[Get_Radiology_Data_ActivityWest]
	*/
...
--	preg.DateRecorded AS PregnancyDateRecorded,
	...
	,P.RadisNumber+'|'+P.pk_Patient_ID +'|'+'West'+'|'+'Radis' as PatientLinkID
	,pg.Code AS ExamGroupCode                           

	
	
FROM
	[RADIS_WEST].RadisReporting.dbo.Patient P		
	      inner join [RADIS_WEST].RadisReporting.dbo.Request AS r WITH ( nolock ) ON p.pk_Patient_ID = r.fk_Patient_ID
            inner join  [RADIS_WEST].RadisReporting.dbo.[Procedure] AS p1 WITH ( nolock ) ON r.pk_Request_ID = p1.fk_Request_ID
            left outer join [RADIS_WEST].RadisReporting.dbo.Appointment AS a WITH ( nolock ) ON a.fk_Procedure_ID = p1.pk_Procedure_ID
            left outer join [RADIS_WEST].RadisReporting.dbo.ReportChapter AS rc WITH ( nolock ) ON r.pk_Request_ID = rc.fk_Request_ID 
            left outer join [RADIS_WEST].RadisReporting.dbo.ReportBatch AS rb WITH ( nolock ) ON rb.pk_ReportBatch_ID = rc.fk_ReportBatch_ID
            left outer join [RADIS_WEST].RadisReporting.dbo.BatchSource AS bs WITH ( nolock ) ON bs.pk_BatchSource_ID = rb.BatchSource 
            left outer join [RADIS_WEST].RadisReporting.dbo.ReportChapterStatus AS rcs WITH ( nolock ) ON rcs.pk_ReportChapterStatus_ID  = rc.fk_ReportChapterStatus_ID
            inner join  [RADIS_WEST].RadisReporting.dbo.ProcedureCode AS pc WITH ( nolock ) ON pc.pk_ProcedureCode_ID = p1.fk_ProcedureCode_ID
            inner join  [RADIS_WEST].RadisReporting.dbo.ProcedureGroup AS pg WITH ( nolock ) ON pg.pk_ProcedureGroup_ID = pc.fk_ProcedureGroup_ID
            inner join  [RADIS_WEST].RadisReporting.dbo.ClinicianPractice AS cp WITH ( nolock ) ON cp.pk_ClinicianPractice_ID = r.fk_ClinicianPractice_Referring_ID
            left outer join [RADIS_WEST].RadisReporting.dbo.Clinician AS c WITH ( nolock ) ON c.pk_Clinician_ID = cp.fk_Clinician_ID
            inner join  [RADIS_WEST].RadisReporting.dbo.Practice AS pr WITH ( nolock ) ON pr.pk_Practice_ID = cp.fk_Practice_ID
            --INNER JOIN  [RADIS_WEST].dbo.SystemSetting WITH ( NOLOCK ) ON [RADIS_WEST].dbo.SystemSetting.SettingName = 'sitename'
            INNER JOIN  [RADIS_WEST].RadisReporting.dbo.RequestStatus AS rs WITH ( NOLOCK ) ON rs.pk_RequestStatus_ID = r.RequestStatus
            INNER JOIN  [RADIS_WEST].RadisReporting.dbo.Site AS s WITH ( NOLOCK ) ON s.pk_Site_ID = r.fk_Site_ID
            INNER JOIN  [RADIS_WEST].RadisReporting.dbo.Specialty AS s1 WITH ( NOLOCK ) ON s1.pk_Specialty_ID = r.fk_Specialty_ID
            INNER JOIN  [RADIS_WEST].RadisReporting.dbo.Location AS l WITH ( NOLOCK ) ON l.pk_Location_ID = r.fk_Location_ID
            INNER JOIN  [RADIS_WEST].RadisReporting.dbo.Priority AS pt WITH ( NOLOCK ) ON pt.pk_Priority_ID = r.Priority
            ...
            LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.Pregnancy AS preg WITH ( NOLOCK ) ON p.fk_Pregnancy_ID = preg.pk_Pregnancy_ID
            LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.PregnancyStatus AS prst WITH ( NOLOCK ) ON preg.fk_PregnancyStatus_ID = prst.pk_PregnancyStatus_ID
          --inner join  [RADIS_WEST].RadisReporting.dbo.Category AS ct WITH ( nolock ) ON ct.pk_Category_ID = r.fk_Category_ID
            LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.Category AS ct WITH ( NOLOCK ) ON ct.pk_Category_ID = r.fk_Category_ID
            LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.RDRoom AS RDR WITH ( NOLOCK ) ON RDR.pk_RDRoom_ID = a.fk_RDRoom_ID
            --LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.Session AS S3 WITH ( NOLOCK ) ON S3.pk_Session_ID = a.fk_Session_ID
            --LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.SessionCode AS SC WITH ( NOLOCK ) ON SC.pk_SessionCode_ID = S3.fk_SessionCode_ID
            ----LEFT OUTER JOIN [RADIS_WEST].dbo.UserDetail AS UD1 WITH ( NOLOCK ) ON UD1.pk_UserDetail_ID = rc.fk_UserDetail_DictationTranscribedBy_ID
            --LEFT OUTER JOIN [RADIS_WEST].dbo.UserDetail AS UD2 WITH ( NOLOCK ) ON UD2.pk_UserDetail_ID = rc.fk_UserDetail_PublishedBy_ID
            LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.ClinicianPractice AS CP2 WITH ( NOLOCK ) ON CP2.pk_ClinicianPractice_ID = p.fk_ClinicianPractice_Registered_ID
            LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.Clinician AS C2 WITH ( NOLOCK ) ON C2.pk_Clinician_ID = CP2.fk_Clinician_ID
            LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.Patient AS Pat2 WITH ( NOLOCK ) ON Pat2.pk_Patient_ID = p.fk_Patient_Primary_ID
            LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.RequestPathologyCode AS RPC WITH ( NOLOCK ) ON RPC.fk_Request_ID = r.pk_Request_ID
            LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.PathologyCode AS pathc WITH ( NOLOCK ) ON pathc.pk_PathologyCode_ID = RPC.fk_PathologyCode_ID
            --INNER JOIN  [RADIS_WEST].dbo.MergedPatientStatus AS MPS WITH ( NOLOCK ) ON MPS.pk_MergedPatientStatus_ID = p.MergedPatientStatus
            LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.PriorityDetail AS pd WITH ( NOLOCK ) ON r.fk_PriorityDetail_ID = pd.pk_PriorityDetail_ID
            LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.Exposure AS e WITH ( NOLOCK ) ON p1.pk_Procedure_ID = e.fk_Procedure_ID
            LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.DoseUnit AS d WITH ( NOLOCK ) ON d.pk_DoseUnit_ID = e.fk_DoseUnit_ID
            LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.Film AS f WITH ( NOLOCK ) ON e.fk_Film_ID = f.pk_Film_ID
            LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.Cancellation AS can WITH ( NOLOCK ) ON r.fk_Cancellation_ID = can.pk_Cancellation_ID
            LEFT OUTER JOIN  [RADIS_WEST].RadisReporting.[dbo].[RadisPostcodeData] rpcd WITH (NOLOCK) ON p.PostCode = rpcd.PostCode
        ...
        --  left outer join NewLHBName nlhb with (nolock) on nhs.fk_NewLHBName_ID = nlhb.pk_NewLHBName_ID
            LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.[Purchaser] AS pur WITH (NOLOCK ) ON pur.pk_Purchaser_ID = r.fk_Purchaser_ID
			LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.Portering AS port WITH (NOLOCK ) ON r.pk_Request_ID = port.fk_Request_ID
            LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.PorteringStatus AS pors WITH (NOLOCK ) ON pors.pk_Porteringstatus_ID = port.fk_Porteringstatus_ID
            LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.Porter AS portcol1 WITH (NOLOCK ) ON portcol1.pk_Porter_ID = port.fk_Porter_Collector1_ID
            LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.Porter AS portcol2 WITH (NOLOCK ) ON portcol2.pk_Porter_ID = port.fk_Porter_Collector2_ID
            LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.Porter AS portcol3 WITH (NOLOCK ) ON portcol3.pk_Porter_ID = port.fk_Porter_Collector3_ID
            LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.Porter AS portcol4 WITH (NOLOCK ) ON portcol4.pk_Porter_ID = port.fk_Porter_Collector4_ID
            LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.Porter AS portret1 WITH (NOLOCK ) ON portret1.pk_Porter_ID = port.fk_Porter_Returnor1_ID
            LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.Porter AS portret2 WITH (NOLOCK ) ON portret2.pk_Porter_ID = port.fk_Porter_Returnor2_ID
            LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.Porter AS portret3 WITH (NOLOCK ) ON portret3.pk_Porter_ID = port.fk_Porter_Returnor3_ID
            LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.Porter AS portret4 WITH (NOLOCK ) ON portret4.pk_Porter_ID = port.fk_Porter_Returnor4_ID
            LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.Abandoned AS abn WITH (NOLOCK) ON abn.pk_Abandoned_ID = r.fk_Abandoned_ID
            LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.UserGroup AS ug WITH (NOLOCK) ON ug.pk_UserGroup_ID = r.fk_UserGroup_Vetting_ID
            LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.UserDetail AS UD3 WITH ( NOLOCK ) ON UD3.pk_UserDetail_ID = r.fk_UserDetail_ReceptionedBy_ID
            LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.UserDetail AS UD4 WITH ( NOLOCK ) ON UD4.pk_UserDetail_ID = a.fk_UserDetail_BookedBy_ID 
			LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.UserDetail AS UD5 WITH ( NOLOCK ) ON UD5.pk_UserDetail_ID = p1.fk_UserDetail_Radiographer_ID
					--LEFT OUTER JOIN [RADIS_WEST].RadisReporting.dbo.UserDetail AS UD6 WITH ( NOLOCK ) ON UD5.pk_UserDetail_ID = rc.fk_UserDetail_Author_ID
WHERE
...
END
GO
GRANT ALTER ON  [dbo].[Get_Radiology_Data_ActivityWest] TO [CYMRU\He105872]
GO
GRANT EXECUTE ON  [dbo].[Get_Radiology_Data_ActivityWest] TO [CYMRU\He105872]
GO
GRANT VIEW DEFINITION ON  [dbo].[Get_Radiology_Data_ActivityWest] TO [CYMRU\He105872]
GO
