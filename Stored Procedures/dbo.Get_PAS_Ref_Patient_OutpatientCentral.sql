...
CREATE PROCEDURE [dbo].[Get_PAS_Ref_Patient_OutpatientCentral]
...
--declare @lastattendancedate as date = '01 january 2019'
DECLARE @LastAttendanceDate AS DATE =	(SELECT 
													...
													FROM [Foundation].[dbo].[PAS_Data_Outpatient] WHERE source='wpas'and AppointmentDate <> '2999-12-31' and AppointmentDate < CONVERT(date,getdate()))
...
''
	SELECT DISTINCT
		...
		''''WPAS'''' AS Source,
		''''OP'''' AS Dataset,
		T.actnotekey||''''|WPAS|OP'''' AS PatientLinkId,
