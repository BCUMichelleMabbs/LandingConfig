...
CREATE PROCEDURE [dbo].[Get_PAS_Data_NonContact_West_HWLv2]
...
WHERE proca.proca_refno = s1.proca_refno
...
) as [HCPAtAppointment], -- s1.proca_refno this is the parnt instance proca HWL

--pro.main_ident AS [HCPAtAppointment],   

ho2.MAIN_IDENT AS [LocationOfAppointment],

dbo.IS_GetRefValMainID(sch.contp_refno) as [TypeOfAppointment],  -- -- oa oc or ot   - what should it be? - dupli  Non Contact AppointmentType also exists  - HWL
dbo.IS_GetRefValMainID(sch.scocm_refno) AS [OutcomeOfAppointment],
...
-- majority of spect_refno is null where sctyp=100207 get specialty via proca_refno for parent instance
...
sch.schdl_refno as [ActNoteKey],  -- scocm of sch applicable so want sch actnotekey and not s1  HWL
dbo.IS_GetRefValMainID(sch.cancr_refno) AS [OutcomeReason],  -- original

null as [IgnorePartialBooking],
CAST('''' AS varchar(100)) as [AppointmentDirective],                --- GET FROM WLIST?
...
dbo.IS_GetRefValMainID(sch.conty_refno) as [ConsultationMethod], ---CONTY_REFNO X6 USED FOR SCTYP=100207
...
CAST('''' as varchar(50)) as [NextApptschdl]
...
FROM schedules sch
LEFT JOIN SCHEDULES s1 on s1.PARNT_REFNO = sch.SCHDL_REFNO AND ISNULL(s1.archv_flag,''N'') = ''N''
...
LEFT JOIN prof_Carers pro on pro.proca_Refno = s1.PROCA_REFNO    -- HWL - this is the parnt instance
...
LEFT JOIN PROF_CARER_SPECIALTIES prospeclink on prospeclink.PROCA_REFNO = s1.PROCA_REFNO AND prospeclink.END_DTTM IS NULL AND ISNULL(prospeclink.archv_flag,''N'') = ''N''	AND prospeclink.CSTYP_REFNO = ''12143''
...
set       NextConsultationMethod = case  
          ...
          NextVirtualContactType = case  
                                    ...
                                    ELSE ''F''
                                   END,
           NextApptschdl = NextAppt.schdl_refno
from #op_table temp 
