...
CREATE PROCEDURE [dbo].[Get_PAS_Ref_ClinicalReview_East]
...
--DECLARE @LastReviewDate as date = '01 January 2020'

--DECLARE @LastReviewDate AS DATE = (SELECT ISNULL(MAX(DateOfReview),'01 January 2010') FROM [Foundation].[dbo].[PAS_Ref_ClinicalReview] where Area = 'East')
...
EXEC( 'SELECT DISTINCT 
		...
		--nullif(rtrim(cast(cr.Review_date as date)), '''') as DateReview2,
		--RIGHT(''0''||CAST(extract(HOUR from cr.Review_Date) AS VARCHAR(2)),2) ||'':''|| RIGHT(''0''||CAST(extract(Minute from cr.Review_Date) AS VARCHAR(2)),2)  as TimeOfReview2,
		--extract(minute from cr.Review_date) as tim3,
		...
		nullif(rtrim(cr.SeqNo), '''') as OrderOfReviews,
		''East'' as Area,
		''Myrddin'' as Source
