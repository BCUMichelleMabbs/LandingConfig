...
SET ANSI_NULLS ON
GO
CREATE Procedure [dbo].[Get_Covid_Data_CentralTransfersOut]
as
begin
		 ...
		 '' '' as TestEntryDate,
		      ...
		      WHERE 
					CAST(CASE
                         WHEN TRANS.TRANSFER_NO = (SELECT MAX(TRANSFER_NO) FROM TRANSFER innerT WHERE innerT.LINKID=TREAT.LINKID) AND TREAT.DISDATE IS NOT NULL THEN TREAT.DISDATE
                         ELSE (SELECT FIRST 1 innerT.TRANSFER_DATE FROM TRANSFER innerT WHERE innerT.LINKID=TRANS.LINKID AND innerT.TRANSFER_NO=TRANS.TRANSFER_NO+1)
                     END AS DATE) >= ''01 November 2019'' -- DATEADD(DAY, -8, CURRENT_TIMESTAMP)
					 AND
					CAST(CASE
                         WHEN TRANS.TRANSFER_NO = (SELECT MAX(TRANSFER_NO) FROM TRANSFER innerT WHERE innerT.LINKID=TREAT.LINKID) AND TREAT.DISDATE IS NOT NULL THEN TREAT.DISDATE
                         ELSE (SELECT FIRST 1 innerT.TRANSFER_DATE FROM TRANSFER innerT WHERE innerT.LINKID=TRANS.LINKID AND innerT.TRANSFER_NO=TRANS.TRANSFER_NO+1)
                     END AS DATE) <= ''YESTERDAY''
...
UNION ALL
			 ...
			 WHERE  
			        CAST(COALESCE(TRANS.TRANSFER_DATE,TREAT.DISDATE) AS DATE) >= ''01 November 2019'' --DATEADD(DAY, -8, CURRENT_TIMESTAMP)
					AND
