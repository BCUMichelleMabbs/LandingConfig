...
CREATE PROCEDURE [dbo].[Get_UnscheduledCare_Data_EDAttendanceEast_OBSOLETE]
...
USE [Wrexham_Live]
declare @flm_mtid int, @NHSid int, @lpi_id int
declare @Diagn int
set @flm_mtid = (select top 1 mt_id from mappingtypes where mt_name = ''EDDS'')
set @NHSid =(select top 1 lkp_id from Lookups where lkp_name = ''NHS Number'')
set @lpi_id=(select top 1 lkp_id from lookups where lkp_name = ''Symphony Patient ID'')
set @Diagn = (select top 1 rps_value from cfg_reportsettings where rps_name = ''Diagnosis'')
...
FROM
	Request_Details R
	INNER JOIN Attendance_Details A ON R.req_atdid=A.atd_id
WHERE
	R.req_depid IN (Select rps_value from cfg_reportsettings where rps_name = ''Radiology'') AND 
...
select res_atdid, res_date, res_field6 from result_details
inner join attendance_details on atd_id = res_atdid
...
select res_atdid, res_date, res_field6 from result_details
inner join attendance_details on res_atdid = atd_id 
where res_depid in (Select rps_value from cfg_reportsettings where rps_name = ''Treatment'')
and res_inactive = 0 
and CAST(atd_arrivaldate AS DATE)> '''+@LastAttendanceDateString+''' AND CAST(atd_arrivaldate AS DATE) < '''+@DateToString+'''
	
insert  @TemporaryTreat_Disch (res_atdid,res_date,res_result)				
select res_atdid, res_date, res_result from result_details
inner join attendance_details on res_atdid = atd_id 
where res_depid in (Select rps_value from cfg_reportsettings where rps_name = ''Treatment'')
and res_inactive = 0 
and CAST(atd_arrivaldate AS DATE)> '''+@LastAttendanceDateString+''' AND CAST(atd_arrivaldate AS DATE) < '''+@DateToString+'''
...
FROM
	Result_details R
	INNER JOIN Attendance_Details A ON R.res_atdid=A.atd_id
WHERE
	R.res_depid IN (Select rps_value from cfg_reportsettings where rps_name = ''DIAGNOSIS'') AND 
...
SELECT DISTINCT
	...
	--[dbo].[FnPatLocalPatientID] (pat_pid) AS LocalPatientIdentifier,
	(SELECT TOP 1 RTRIM(psi_system_id) FROM Patient_system_ids PSID WHERE PSID.psi_system_name=@lpi_id AND psi_pid=pat_pid) AS LocalPatientIdentifier,
	NULLIF(RTRIM((SELECT TOP 1 LEFT(psi_system_id,10) FROM patient_System_ids WHERE psi_system_name = @NHSid AND psi_pid=pat_pid)),'''') AS NHSNumber,
	...
	CASE 
		WHEN PermAdd.add_postcode IS NULL THEN (SELECT TOP 1 left(ha_pcg,3) FROM ha_pcg WHERE TempAdd.add_postcode=ha_pcg.ha_postcode )
		ELSE(SELECT TOP 1 LEFT(ha_pcg,3) FROM ha_pcg WHERE PermAdd.add_postcode=ha_pcg.ha_postcode )
	END AS DHA,
	...
	CAST((SELECT TOP 1 tri_date FROM triage WHERE tri_atdid = a.atd_id AND tri_inactive <> 1 ORDER BY tri_date ASC) AS DATE) AS TriageStartDate, 
	CAST((SELECT TOP 1 tri_date FROM triage WHERE tri_atdid = a.atd_id AND tri_inactive <> 1 ORDER BY tri_date ASC) AS TIME) AS TriageStartTime,
	NULL AS TriageEndDate,
	NULL AS TriageEndTime, 
	CAST((SELECT TOP 1 res_date FROM result_details WHERE res_depid IN (SELECT rps_value FROM cfg_reportsettings WHERE rps_name = ''SeeA+EClinicianOutcome'') AND res_atdid = a.atd_id AND res_inactive<>1) AS DATE) AS EDClinicianSeenDate,
	CAST((SELECT TOP 1 res_date FROM result_details WHERE res_depid IN (SELECT rps_value FROM cfg_reportsettings WHERE rps_name = ''SeeA+EClinicianOutcome'') AND res_atdid = a.atd_id AND res_inactive<>1) AS TIME) AS EDClinicianSeenTime,
	...
	CAST((SELECT TOP (1) res_date FROM Result_details WHERE (res_depid = (SELECT dep_id FROM CFG_DEProcedures WHERE (dep_Caption = ''Breach Exception''))) AND res_atdid = a.atd_id AND res_inactive <> 1) AS DATE) AS TreatmentCompleteDate,
	CAST((SELECT TOP (1) res_date FROM Result_details WHERE (res_depid = (SELECT dep_id FROM CFG_DEProcedures WHERE (dep_Caption = ''Breach Exception''))) AND res_atdid = a.atd_id AND res_inactive <> 1) AS TIME) AS TreatmentCompleteTime,
	...
	CAST((SELECT TOP (1) req_date FROM Request_Details WHERE (req_depid IN (SELECT dep_id FROM CFG_DEProcedures WHERE (dep_Caption = ''DTA''))) AND req_atdid = a.atd_id AND req_inactive <> 1) AS DATE) AS DecisionToAdmitDate,
	CAST((SELECT TOP (1) req_date FROM Request_Details WHERE (req_depid IN (SELECT dep_id FROM CFG_DEProcedures WHERE (dep_Caption = ''DTA''))) AND req_atdid = a.atd_id AND req_inactive <> 1) AS TIME) AS DecisionToAdmitTime,
	...
	NULLIF(CAST((
		CASE a.atd_attendancetype 
			...
			ELSE (SELECT TOP 1 cul_locationdate FROM current_locations WHERE cul_locationid IN (SELECT TOP 1 loc_id FROM cfg_locations WHERE loc_name = ''Left Department'') AND cul_atdid = a.atd_id ORDER BY cul_locationdate DESC)
		END
	...
	CAST((SELECT TOP (1) res_date FROM Result_details AS Trans WHERE (res_depid IN (SELECT rps_value FROM CFG_ReportSettings AS CFG_ReportSettings_5 WHERE (rps_name = ''Transport''))) AND res_atdid = A.atd_id AND res_inactive <> 1) AS DATE) AS TransportRequestedDate,
	CAST((SELECT TOP (1) res_date FROM Result_details AS Trans WHERE (res_depid IN (SELECT rps_value FROM CFG_ReportSettings AS CFG_ReportSettings_5 WHERE (rps_name = ''Transport''))) AND res_atdid = A.atd_id AND res_inactive <> 1) AS TIME) AS TransportRequestedTime,
	NULL AS TransportArrivedDate,  
	NULL AS TransportArrivedTime,
	CAST((SELECT TOP (1) req_date FROM Request_Details WHERE (req_depid IN (SELECT rps_value FROM CFG_ReportSettings WHERE (rps_name = ''A+E BedRequest''))) AND req_atdid = a.atd_id AND req_inactive <> 1) AS DATE) AS BedRequestedDate,
	CAST((SELECT TOP (1) req_date FROM Request_Details WHERE (req_depid IN (SELECT rps_value FROM CFG_ReportSettings WHERE (rps_name = ''A+E BedRequest''))) AND req_atdid = a.atd_id AND req_inactive <> 1) AS TIME) AS BedRequestedTime,
	CAST((SELECT TOP (1) res_date FROM Result_details WHERE (res_depid IN (SELECT rps_value FROM CFG_ReportSettings WHERE (rps_name = ''A+E BedRequestOutcome''))) AND res_atdid = a.atd_id AND res_inactive <> 1) AS DATE) AS BedRequestedOutcomeDate,
	CAST((SELECT TOP (1) res_date FROM Result_details WHERE (res_depid IN (SELECT rps_value FROM CFG_ReportSettings WHERE (rps_name = ''A+E BedRequestOutcome''))) AND res_atdid = a.atd_id AND res_inactive <> 1) AS TIME) AS BedRequestedOutcomeTime,
	...
	--NULLIF(a.atd_reason,0) AS PatientGroup,  --The actual PatientGroup field is mostly blank and the values for reason better match what WPAS uses for PatientGroup - also, the MDS query uses reason as well
	--CASE a.atd_attendancetype 
	--	WHEN 1 THEN ISNULL(
	--		(select distinct not_text
	--			from 
	--			attendance_details innerA1
	--			inner join episodes on atd_epdid = epd_id 
	--			inner join lookupmappings on flm_lkpid = atd_reason
	--			inner join notes on not_noteid = flm_value
	--		where 
	--			innerA1.atd_attendancetype <> 1 and 
	--			innerA1.atd_id <> 0 and 
	--			innerA1.atd_deleted<>1 and 
	--			right(innerA1.atd_num,1)=''1'' AND 
	--			innerA1.atd_id=a.atd_id and 
	--			CAST(a.atd_arrivaldate AS DATE)> '''+@LastAttendanceDateString+''' AND CAST(a.atd_arrivaldate AS DATE) < '''+@DateToString+''')
	--		,
	--		(select distinct not_text
	--			from 
	--			attendance_details innerA2
	--			inner join episodes on atd_epdid = epd_id 
	--			inner join lookupmappings on flm_lkpid = atd_reason
	--			inner join notes on not_noteid = flm_value
	--		where 
	...
	--			innerA2.atd_id=a.atd_id and 
	--			CAST(innerA2.atd_arrivaldate AS DATE)> '''+@LastAttendanceDateString+''' AND CAST(innerA2.atd_arrivaldate AS DATE) < '''+@DateToString+''')
	--		)
	--ELSE LEFT((select top 1 not_text from notes where not_noteid = (select top 1 flm_value from lookupmappings where flm_lkpid = a.atd_reason and flm_mtid = 6)), 2) 
 --end as PatientGroup,
	...
	CASE
		WHEN (select not_text from notes where not_noteid = (select top 1 flm_value from lookupmappings where flm_lkpid = a.atd_reason and flm_mtid = @flm_mtid)) not in (11,12,13,14,15) THEN ''98''
		ELSE
		ISNULL(
			CASE
				WHEN (select top 1 lkp_name from lookups where lkp_id = a.atd_activity) in 
				...
				WHEN (select top 1 lkp_name from lookups where lkp_id = a.atd_activity) = ''Miscellaneous'' THEN ''08''
				WHEN (select top 1 lkp_name from lookups where lkp_id = (select top 1 rta_patientype from RTA_Details where rta_atdid = a.atd_id)) in
				...
				WHEN (select top 1 lkp_name from lookups where lkp_id = a.atd_location) = ''Work Place'' THEN ''01''
				WHEN (select top 1 lkp_name from lookups where lkp_id = a.atd_location) = ''Educational Establishment'' THEN ''02''
				WHEN (select top 1 lkp_name from lookups where lkp_id = a.atd_location) in
				...
				WHEN (select top 1 lkp_name from lookups where lkp_id = a.atd_location) = ''UNKNOWN'' THEN ''98''
				WHEN (select top 1 lkp_name from lookups where lkp_id = a.atd_location) in
	...
	(SELECT TOP 1 tri_category FROM triage WHERE tri_atdid = a.atd_id AND tri_inactive <> 1 ORDER BY tri_date ASC) AS TriageCategory, 

	NULLIF((SELECT TOP 1 tri_complaint FROM triage WHERE tri_atdid = a.atd_id AND tri_inactive <> 1 ORDER BY tri_date ASC),'''') AS TriageComplaint, 
	NULLIF(RTRIM((SELECT TOP 1 tri_treatment FROM triage WHERE tri_atdid = a.atd_id AND tri_inactive <> 1 ORDER BY tri_date ASC)),'''') AS TriageTreatment,
	--THIS COULD PROLLY BE OPTIMISED BY PULLING THE FULL FIELD OUT AT THE BEGINNING AND DOING A PARTITION OVER
	NULLIF(LEFT((SELECT TOP 1 tri_treatment FROM triage WHERE tri_atdid = a.atd_id AND tri_inactive <> 1 ORDER BY tri_date ASC),4),0) TriageTreatment1,
	NULLIF(SUBSTRING((SELECT TOP 1 tri_treatment FROM triage WHERE tri_atdid = a.atd_id AND tri_inactive <> 1 ORDER BY tri_date ASC),7,4),0) AS TriageTreatment2,
	NULLIF(SUBSTRING((SELECT TOP 1 tri_treatment FROM triage WHERE tri_atdid = a.atd_id AND tri_inactive <> 1 ORDER BY tri_date ASC),13,4),0) AS TriageTreatment3,
	NULLIF(SUBSTRING((SELECT TOP 1 tri_treatment FROM triage WHERE tri_atdid = a.atd_id AND tri_inactive <> 1 ORDER BY tri_date ASC),19,4),0) AS TriageTreatment4,
	NULLIF(SUBSTRING((SELECT TOP 1 tri_treatment FROM triage WHERE tri_atdid = a.atd_id AND tri_inactive <> 1 ORDER BY tri_date ASC),25,4),0) AS TriageTreatment5,
	NULLIF(SUBSTRING((SELECT TOP 1 tri_treatment FROM triage WHERE tri_atdid = a.atd_id AND tri_inactive <> 1 ORDER BY tri_date ASC),31,4),0) AS TriageTreatment6,
	NULLIF(SUBSTRING((SELECT TOP 1 tri_treatment FROM triage WHERE tri_atdid = a.atd_id AND tri_inactive <> 1 ORDER BY tri_date ASC),37,4),0) AS TriageTreatment7,
	NULLIF(SUBSTRING((SELECT TOP 1 tri_treatment FROM triage WHERE tri_atdid = a.atd_id AND tri_inactive <> 1 ORDER BY tri_date ASC),43,4),0) AS TriageTreatment8,
	NULLIF(SUBSTRING((SELECT TOP 1 tri_treatment FROM triage WHERE tri_atdid = a.atd_id AND tri_inactive <> 1 ORDER BY tri_date ASC),49,4),0) AS TriageTreatment9,
	NULLIF(SUBSTRING((SELECT TOP 1 tri_treatment FROM triage WHERE tri_atdid = a.atd_id AND tri_inactive <> 1 ORDER BY tri_date ASC),55,4),0) AS TriageTreatment10,
	NULLIF(RTRIM((SELECT TOP 1 tri_discriminator FROM triage WHERE tri_atdid = a.atd_id AND tri_inactive <> 1 ORDER BY tri_date ASC)),'''') AS TriageDiscriminator, 
	...
	CASE
		...
		WHEN (
			SELECT 
				TOP 1 res_result 
			FROM  Result_details 
				INNER JOIN lookups ON lkp_id = res_result 
			WHERE 
					res_inactive<>1 AND 
	...
	--a.atd_patgroup AS PatientGroup,  --Mostly blank
	(SELECT TOP (1) req_field1 FROM Request_Details WHERE (req_depid IN (SELECT rps_value FROM CFG_ReportSettings WHERE (rps_name = ''A+E BedRequest''))) AND req_atdid = a.atd_id AND req_inactive <> 1) AS BedRequestedWard,
	NULL AS BedRequestedConsultant,
	(SELECT TOP (1) req_request FROM Request_Details WHERE (req_depid IN (SELECT rps_value FROM CFG_ReportSettings WHERE (rps_name = ''A+E BedRequest''))) AND req_atdid = a.atd_id AND req_inactive <> 1) AS BedRequestedSpecialty,
	(SELECT TOP (1) res_result FROM Result_details WHERE (res_depid IN (SELECT rps_value FROM CFG_ReportSettings WHERE (rps_name = ''A+E BedRequestOutcome''))) AND res_atdid = a.atd_id AND res_inactive <> 1) AS BedRequestedOutcome,
	(SELECT TOP 1 res_staff1 FROM result_details WHERE res_depid IN (SELECT rps_value FROM cfg_reportsettings WHERE rps_name = ''SeeA+EClinicianOutcome'') AND res_atdid = a.atd_id AND res_inactive<>1) AS EDClinicianSeen,
...
FROM 
	...
	INNER JOIN episodes on a.atd_epdid = epd_id 
	INNER JOIN patient on epd_pid = pat_pid 
	INNER JOIN Patient_details on pdt_pid=pat_pid 
	--LEFT JOIN Patient_Details_ExtraFields PDEF on patient.pat_pid=PDEF.pdt_EF_pdtpid and PDEF.pdt_EF_FieldID = 1050 --School
	LEFT OUTER JOIN GPPracticeAddressLink GPLINK on pdt_gpid = GPLINK.gpa_gpid 
	   AND GPLINK.gpa_status = 2661   -- Status = Active
	   AND gpa_prid = pdt_practise   -- link on GP Practice as well
	LEFT OUTER JOIN GP_practise GPPRAC on GPPRAC.pr_id = GPLINK.gpa_prid
	LEFT OUTER JOIN GP PATGP on PATGP.gp_id = GPLINK.gpa_gpid
	LEFT OUTER JOIN Address GPAdd on GPLINK.gpa_praddid = GPAdd.add_addid 
		AND GPAdd.add_linktype = 2 
		AND GPAdd.add_type IN 
			(
				(SELECT TOP 1 Lkp_ID FROM lookups WHERE Lookups.Lkp_Name = ''Principle Practice Address'' AND lkp_parentid = (SELECT TOP 1 lkp_id FROM lookups WHERE lkp_name = ''Practice Address Type'' AND lkp_parentid = 0)),
				(SELECT TOP 1 Lkp_ID FROM lookups WHERE Lookups.Lkp_Name = ''Secondary Practice Address'' AND lkp_parentid = (SELECT TOP 1 lkp_id FROM lookups WHERE lkp_name = ''Practice Address Type'' AND lkp_parentid = 0)),
				(SELECT TOP 1 Lkp_ID FROM lookups WHERE Lookups.Lkp_Name = ''GP Address''))
	LEFT OUTER JOIN GP REFGP ON REFGP.gp_id = a.atd_gpid
	LEFT OUTER JOIN GP_Practise REFGPPRAC ON REFGPPRAC.Pr_id = a.atd_prcode
	LEFT OUTER JOIN Address ContAdd ON ContAdd.add_linkid = pat_pid AND ContAdd.add_linktype = 1 AND ContAdd.add_type = (SELECT TOP 1 Lkp_ID FROM lookups WHERE Lookups.Lkp_Name = ''Contact Address'')
	LEFT OUTER JOIN Address TempAdd ON TempAdd.add_linkid = pat_pid AND TempAdd.add_linktype = 1 AND TempAdd.add_type = (SELECT TOP 1 Lkp_ID FROM lookups WHERE Lookups.Lkp_Name = ''Temporary address'')
	LEFT OUTER JOIN Address PermAdd ON PermAdd.add_linkid = pat_pid AND PermAdd.add_linktype = 1 AND PermAdd.add_type = (SELECT TOP 1 Lkp_ID FROM lookups WHERE Lookups.Lkp_Name = ''Permanent address'')
	LEFT JOIN RTA_Details RTA ON a.atd_id=RTA.rta_atdid
	LEFT JOIN Attendance_Details_ExtraFields MECH ON a.atd_id=MECH.atd_EF_atdid AND MECH.atd_EF_FieldID=1070
