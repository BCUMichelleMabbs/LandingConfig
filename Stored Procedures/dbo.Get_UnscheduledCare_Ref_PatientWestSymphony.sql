...
CREATE PROCEDURE [dbo].[Get_UnscheduledCare_Ref_PatientWestSymphony]
...
DECLARE @LastAttendanceDate AS DATE = DATEADD(DAY,-7,GETDATE())--(SELECT ISNULL(MAX(ArrivalDate),'31 December 2019') FROM [Foundation].[dbo].[UnscheduledCare_Data_EDAttendance] WHERE Source='WEDS' AND Area='West')-- AND SiteCodeOfTreatment='7A1AU')
