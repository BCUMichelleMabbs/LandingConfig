...
-- =============================================
CREATE PROCEDURE [dbo].[Get_PAS_Data_Outpatient_Central]
...
BEGIN
	...
	SET NOCOUNT ON;
	 ...
	 -- Prior to November 16 the date was on PAS and is held in the old warehouse.

	DECLARE @LastTreatmentDate AS DATE = (SELECT ISNULL(MAX(AppointmentDate),'21 November 2016') FROM [Foundation].[dbo].[PAS_Data_Outpatient] WHERE Source ='WPAS' and AppointmentDate <> '2999-12-31' and AppointmentDate < CONVERT(date,getdate()))
--DECLARE @LastTreatmentDate AS DATE = '31 Dec 2016'
	...
	ELSE ''1'' END as AttendanceCategory,
     ...
     (P.ACTNOTEKEY = T.ACTNOTEKEY)
		...
		WHERE
			...
			--t.trt_Date between ''01 january 2017'' and ''31 december 2018''
